...
export class PostService {
  ...
  protected convertDateFromClient(post: IPost): IPost {
    return Object.assign({}, post, {
      like: post.like?.isValid() ? post.like.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.like = res.body.like ? dayjs(res.body.like) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((post: IPost) => {
        post.like = post.like ? dayjs(post.like) : undefined;
      });
    }
    return res;
  }
